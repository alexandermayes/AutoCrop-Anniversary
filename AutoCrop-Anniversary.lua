--saved variables
if not AutoCropDB or (AutoCropDB.version ~= "2.1" and AutoCropDB.version ~= "2.1.1" and AutoCropDB.version ~= "2.1.2") then
    AutoCropDB = {
...
--trinket ids
local availableTrinketIDs = {25653, 32863}
--goggle ids
local availableGogglesIDs = {34354, 35182, 34847, 34353, 34356, 35184, 34355, 35185, 35181, 35183, 32480, 32474, 32472, 32476, 32461, 32494, 32478, 32475, 32479, 34357, 32495, 32473, 23762}

local isDruid = false
local isFlying = false
local ridingGearEquipped = false
local caughtInCombat = false
local zones = {}
...
function AutoCropSearchInventory()
    ...
    for slot = 0, C_Container.GetContainerNumSlots(bag) do
      ...
      if(link) then
        ...
        if(AutoCropDB.legacyEnabled) then
          ...
          if(AutoCropInArray(itemID, availableTrinketIDs)) then
            AutoCropDB.ridingTrinketID = itemID
          elseif(AutoCropInArray(itemID, availableGogglesIDs)) then
  ...
  if(AutoCropInArray(itemID, availableGogglesIDs)) then
    AutoCropDB.gogglesID = itemID
  end
end
...
f:SetScript('OnEvent', function(self, event, ...)
  ...
  if(event == "ADDON_LOADED") then
    ...
    if addon == "AutoCrop-Anniversary" then
      ...
      if(language == "frFR") then zones = {"Vallee d'Ombrelune","Nagrand","Raz-de-Neant","Marecage de Zangar"}
        elseif(language == "deDE") then zones = {"Schattenmondtal","Nagrand","Nethersturm","Zangarmarschen"}
        elseif(language == "esES" or language == "esMX") then zones = {"Valle Sombraluna","Nagrand","Tormenta Abisal","Marisma de Zangar"}
        ...
        else zones = {"Shadowmoon Valley","Nagrand","Netherstorm","Zangarmarsh"}
      end
      --riding gear check
      ridingGearEquipped = false
...
local function OnSlash(key, value, ...)
  if key and key ~= "" then
    ...
    elseif key == "button" then
      ...
      elseif value == "scale" then
        ...
        if tonumber(arg2) then
          AutoCropDB.buttonScale = tonumber(arg2)
    ...
    AutoCropPrint("slot - toggle trinket slot you want to use. Values: 13(upper)/14(lower) ("..AutoCropDB.trinketSlot..")")
